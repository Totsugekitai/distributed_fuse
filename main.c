...
static void *dfuse_init(struct fuse_conn_info *conn, struct fuse_config *fconfig)
{
    (void)conn;
    (void)fconfig;
    return (NULL);
...
static int dfuse_getattr(const char *path, struct stat *st, struct fuse_file_info *fi)
{
    (void)fi;
...
static int dfuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
                         off_t offset, struct fuse_file_info *fi, enum fuse_readdir_flags flg)
{
    (void)flg;
    (void)offset;
    (void)fi;
...
static int dfuse_read(const char *path, char *buf, size_t size, off_t off,
